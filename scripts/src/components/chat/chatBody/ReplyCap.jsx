...
const ReplyCap = memo(function ReplyCap({ replyTo, onCancelReply }) {
  const currentReactionsUser = JSON.parse(localStorage.getItem('userData'));
  const currentUser = currentReactionsUser ? { id: currentReactionsUser.id, name: currentReactionsUser.name } : null;
