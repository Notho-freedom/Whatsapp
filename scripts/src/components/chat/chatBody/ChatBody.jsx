...
import { useAppReducer} from '@/context/AppReducerContext';
...
const ChatBody = memo(function ChatBody({ selectedChat}) {
  ...
  const currentUser = typeof window !== 'undefined' && localStorage.getItem('userData')
  ...
  useEffect(() => {
    if (selectedChat && messages[selectedChat.id]) {
      actions.markMessagesRead(selectedChat.id);
      
      // Marquer les messages comme lus en temps réel
      const chatMessages = messages[selectedChat.id] || [];
      chatMessages.forEach(message => {
        if (message.sender !== currentUserId && !message.read) {
          actions.markMessageAsRead(selectedChat.id, message.id);
        }
      });
    }
  }, [selectedChat?.id, actions.markMessagesRead]);
